...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/sydneylambert/Desktop/pig/mrs-m/experiment/ses-3_run-3_lastrun.py',
...
iti_old_3 = visual.TextStim(win=win, name='iti_old_3',
    text='1\nOld',
    font='Open Sans',
    pos=(-.15, -.325), height=0.03, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
    depth=0.0);
iti_new_3 = visual.TextStim(win=win, name='iti_new_3',
    text='2\nNew',
    font='Open Sans',
    pos=(.15, -.325), height=0.03, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
...
bps2_trials = data.TrialHandler(nReps=1.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions(f'data/{bidsID}/{bidsID}_ses-3_task-bps2_events-input.csv'),
    seed=None, name='bps2_trials')
thisExp.addLoop(bps2_trials)  # add the loop to the experiment
thisBps2_trial = bps2_trials.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisBps2_trial.rgb)
if thisBps2_trial != None:
    for paramName in thisBps2_trial:
        exec('{} = thisBps2_trial[paramName]'.format(paramName))

for thisBps2_trial in bps2_trials:
    currentLoop = bps2_trials
    # abbreviate parameter names if possible (e.g. rgb = thisBps2_trial.rgb)
    if thisBps2_trial != None:
        for paramName in thisBps2_trial:
            exec('{} = thisBps2_trial[paramName]'.format(paramName))
    ...
    bps2_trials.addData('trials_resp.keys',trials_resp.keys)
    if trials_resp.keys != None:  # we had a response
        bps2_trials.addData('trials_resp.rt', trials_resp.rt)
        bps2_trials.addData('trials_resp.duration', trials_resp.duration)
...
# completed 1.0 repeats of 'bps2_trials'
...
# update component parameters for each repeat
# keep track of which components have finished
final_itiComponents = [iti_old_3, iti_new_3, iti_final]
...
while continueRoutine and routineTimer.getTime() < 10.0:
    ...
    # if iti_old_3 is starting this frame...
    if iti_old_3.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        iti_old_3.frameNStart = frameN  # exact frame index
        iti_old_3.tStart = t  # local t and not account for scr refresh
        iti_old_3.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(iti_old_3, 'tStartRefresh')  # time at next scr refresh
        # update status
        iti_old_3.status = STARTED
        iti_old_3.setAutoDraw(True)
    
    # if iti_old_3 is active this frame...
    if iti_old_3.status == STARTED:
        # update params
        pass
    
    # if iti_old_3 is stopping this frame...
    if iti_old_3.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > iti_old_3.tStartRefresh + 10-frameTolerance:
            # keep track of stop time/frame for later
            iti_old_3.tStop = t  # not accounting for scr refresh
            iti_old_3.frameNStop = frameN  # exact frame index
            # update status
            iti_old_3.status = FINISHED
            iti_old_3.setAutoDraw(False)
    
    # *iti_new_3* updates
    
    # if iti_new_3 is starting this frame...
    if iti_new_3.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        iti_new_3.frameNStart = frameN  # exact frame index
        iti_new_3.tStart = t  # local t and not account for scr refresh
        iti_new_3.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(iti_new_3, 'tStartRefresh')  # time at next scr refresh
        # update status
        iti_new_3.status = STARTED
        iti_new_3.setAutoDraw(True)
    
    # if iti_new_3 is active this frame...
    if iti_new_3.status == STARTED:
        # update params
        pass
    
    # if iti_new_3 is stopping this frame...
    if iti_new_3.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > iti_new_3.tStartRefresh + 10-frameTolerance:
            # keep track of stop time/frame for later
            iti_new_3.tStop = t  # not accounting for scr refresh
            iti_new_3.frameNStop = frameN  # exact frame index
            # update status
            iti_new_3.status = FINISHED
            iti_new_3.setAutoDraw(False)
