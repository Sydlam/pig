...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/sydneylambert/Desktop/pig/experiment/ses-1_run-1_lastrun.py',
...
cond_trials = data.TrialHandler(nReps=1.0, method='sequential', 
    ...
    trialList=data.importConditions(f'data/{bidsID}/{bidsID}_ses-1_task-acq_events-input.csv'),
...
for thisCond_trial in cond_trials:
    ...
    # keep track of which components have finished
    trialComponents = [trials_resp, animal, tool, cond_image]
    ...
    while continueRoutine:
        ...
        # if cond_image is starting this frame...
        if cond_image.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            cond_image.frameNStart = frameN  # exact frame index
            cond_image.tStart = t  # local t and not account for scr refresh
            cond_image.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(cond_image, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'cond_image.started')
            # update status
            cond_image.status = STARTED
            cond_image.setAutoDraw(True)
        
        # if cond_image is active this frame...
        if cond_image.status == STARTED:
            # update params
            pass
        
        # if cond_image is stopping this frame...
        if cond_image.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > cond_image.tStartRefresh + cs_duration-frameTolerance:
                # keep track of stop time/frame for later
                cond_image.tStop = t  # not accounting for scr refresh
                cond_image.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'cond_image.stopped')
                # update status
                cond_image.status = FINISHED
                cond_image.setAutoDraw(False)
    ...
    if trials_resp.keys != None:  # we had a response
        ...
        cond_trials.addData('trials_resp.duration', trials_resp.duration)
