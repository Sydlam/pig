...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/sydneylambert/Desktop/pig/mrs-m/experiment/ses-1_run-3_lastrun.py',
...
ext_trials = data.TrialHandler(nReps=1.0, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions(f'data/{bidsID}/{bidsID}_ses-1_task-ext_events-input.csv'),
    seed=None, name='ext_trials')
thisExp.addLoop(ext_trials)  # add the loop to the experiment
thisExt_trial = ext_trials.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisExt_trial.rgb)
if thisExt_trial != None:
    for paramName in thisExt_trial:
        exec('{} = thisExt_trial[paramName]'.format(paramName))

for thisExt_trial in ext_trials:
    currentLoop = ext_trials
    # abbreviate parameter names if possible (e.g. rgb = thisExt_trial.rgb)
    if thisExt_trial != None:
        for paramName in thisExt_trial:
            exec('{} = thisExt_trial[paramName]'.format(paramName))
    ...
    ext_trials.addData('trials_resp.keys',trials_resp.keys)
    if trials_resp.keys != None:  # we had a response
        ext_trials.addData('trials_resp.rt', trials_resp.rt)
        ext_trials.addData('trials_resp.duration', trials_resp.duration)
...
# completed 1.0 repeats of 'ext_trials'
...
date = expInfo["date"]
filename = f"data/{bidsID}/{bidsID}_ses-1_task-{expName}_{date}"
...
# 🔥 Delete extra _timestamped files
unwanted_pattern = f"data/{bidsID}/{bidsID}_ses-1_task-{expName}_{date}_*.csv"
unwanted_files = glob.glob(unwanted_pattern)
for f in unwanted_files:
    if f != filename + '.csv':  # don't delete the good one
        os.remove(f)
