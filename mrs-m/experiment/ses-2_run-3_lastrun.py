...
expName = 'sourcetyp'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/sydneylambert/Desktop/pig/mrs-m/experiment/ses-2_run-3_lastrun.py',
...
pracsou_slider = visual.Slider(win=win, name='pracsou_slider',
    startValue=None, size=(0.35, 0.05), pos=(0, -0.325), units=win.units,
    labels=['Phase 1', 'Phase 2', 'Phase 3'],ticks=None, granularity=1,
...
practyp_slider = visual.Slider(win=win, name='practyp_slider',
    startValue=None, size=(0.6, 0.05), pos=(0, -0.325), units=win.units,
    labels=['Not', 'Somewhat', 'Very'],ticks=None, granularity=1,
    style='radio', styleTweaks=(), opacity=None,
    labelColor='LightGray', markerColor='Red', lineColor='White', colorSpace='rgb',
    font='Open Sans', labelHeight=0.03,
...
while continueRoutine:
    ...
    # if inst5 is stopping this frame...
    if inst5.status == STARTED:
...
if inst5_resp.keys != None:  # we had a response
    ...
    thisExp.addData('inst5_resp.duration', inst5_resp.duration)
thisExp.nextEntry()
...
while continueRoutine:
    ...
    # if inst8 is starting this frame...
    if inst8.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        ...
        # update status
        inst8.status = STARTED
        inst8.setAutoDraw(True)
    
    # if inst8 is active this frame...
    if inst8.status == STARTED:
        # update params
        pass
    
    # if inst8 is stopping this frame...
    if inst8.status == STARTED:
        if bool(inst8_resp.keys=='space'):
            ...
            # update status
            inst8.status = FINISHED
            inst8.setAutoDraw(False)
    
    # *inst8_resp* updates
    waitOnFlip = False
    
    # if inst8_resp is starting this frame...
    ...
    # if inst8_resp is stopping this frame...
    ...
    if inst8_resp.status == STARTED and not waitOnFlip:
        ...
        if len(_inst8_resp_allKeys):
            ...
            inst8_resp.duration = _inst8_resp_allKeys[-1].duration
            # a response ends the routine
            continueRoutine = False
...
if inst8_resp.keys != None:  # we had a response
    ...
    thisExp.addData('inst8_resp.duration', inst8_resp.duration)
thisExp.nextEntry()
...
while continueRoutine:
    ...
    # if inst_practrials is starting this frame...
    if inst_practrials.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        ...
        # update status
        inst_practrials.status = STARTED
        inst_practrials.setAutoDraw(True)
    
    # if inst_practrials is active this frame...
    if inst_practrials.status == STARTED:
        # update params
        pass
    
    # if inst_practrials is stopping this frame...
    if inst_practrials.status == STARTED:
        if bool(inst_practrials_resp.keys=='space'):
            ...
            # update status
            inst_practrials.status = FINISHED
            inst_practrials.setAutoDraw(False)
    
    # *inst_practrials_resp* updates
    waitOnFlip = False
    
    # if inst_practrials_resp is starting this frame...
    ...
    # if inst_practrials_resp is stopping this frame...
    ...
    if inst_practrials_resp.status == STARTED and not waitOnFlip:
        ...
        if len(_inst_practrials_resp_allKeys):
            ...
            inst_practrials_resp.duration = _inst_practrials_resp_allKeys[-1].duration
            # a response ends the routine
            continueRoutine = False
...
if inst_practrials_resp.keys != None:  # we had a response
    ...
    thisExp.addData('inst_practrials_resp.duration', inst_practrials_resp.duration)
thisExp.nextEntry()
...
while continueRoutine and routineTimer.getTime() < 1.0:
    ...
    # if prac_crosshair is starting this frame...
    if prac_crosshair.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        ...
        # update status
        prac_crosshair.status = STARTED
        prac_crosshair.setAutoDraw(True)
    
    # if prac_crosshair is active this frame...
    if prac_crosshair.status == STARTED:
        # update params
        pass
    
    # if prac_crosshair is stopping this frame...
    if prac_crosshair.status == STARTED:
        ...
        if tThisFlipGlobal > prac_crosshair.tStartRefresh + 1.0-frameTolerance:
            ...
            # update status
            prac_crosshair.status = FINISHED
...
while continueRoutine:
    ...
    # if prac_when is starting this frame...
    if prac_when.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        ...
        # update status
        prac_when.status = STARTED
        prac_when.setAutoDraw(True)
    
    # if prac_when is active this frame...
    if prac_when.status == STARTED:
        # update params
        pass
    
    # *pracwhen_image* updates
    
    # if pracwhen_image is starting this frame...
    if pracwhen_image.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        ...
        # update status
        pracwhen_image.status = STARTED
        pracwhen_image.setAutoDraw(True)
    
    # if pracwhen_image is active this frame...
    if pracwhen_image.status == STARTED:
        # update params
        pass
    
    # *pracsou_slider* updates
    
    # if pracsou_slider is starting this frame...
    if pracsou_slider.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        ...
        # update status
        pracsou_slider.status = STARTED
        pracsou_slider.setAutoDraw(True)
    
    # if pracsou_slider is active this frame...
    if pracsou_slider.status == STARTED:
        # update params
        pass
...
while continueRoutine:
    ...
    # if soufeedback_resp is starting this frame...
    ...
    # if soufeedback_resp is stopping this frame...
    ...
    if soufeedback_resp.status == STARTED and not waitOnFlip:
        ...
        if len(_soufeedback_resp_allKeys):
            ...
            soufeedback_resp.duration = _soufeedback_resp_allKeys[-1].duration
            # a response ends the routine
            continueRoutine = False
    
    # *soufeedback_text* updates
    
    # if soufeedback_text is starting this frame...
    if soufeedback_text.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        ...
        # update status
        soufeedback_text.status = STARTED
        soufeedback_text.setAutoDraw(True)
    
    # if soufeedback_text is active this frame...
    if soufeedback_text.status == STARTED:
        # update params
        pass
    
    # if soufeedback_text is stopping this frame...
    if soufeedback_text.status == STARTED:
        if bool(soufeedback_resp.keys=='space'):
            # keep track of stop time/frame for later
            ...
            # update status
            soufeedback_text.status = FINISHED
...
if soufeedback_resp.keys != None:  # we had a response
    ...
    thisExp.addData('soufeedback_resp.duration', soufeedback_resp.duration)
thisExp.nextEntry()
...
while continueRoutine:
    ...
    # if practyp_image is starting this frame...
    if practyp_image.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        ...
        # update status
        practyp_image.status = STARTED
        practyp_image.setAutoDraw(True)
    
    # if practyp_image is active this frame...
    if practyp_image.status == STARTED:
        # update params
        pass
    ...
    # if practyp_resp is starting this frame...
    ...
    if practyp_resp.status == STARTED and not waitOnFlip:
        ...
        if len(_practyp_resp_allKeys):
            ...
            practyp_resp.duration = _practyp_resp_allKeys[-1].duration
            # a response ends the routine
            continueRoutine = False
    
    # *practyp_question* updates
    
    # if practyp_question is starting this frame...
    if practyp_question.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        ...
        # update status
        practyp_question.status = STARTED
        practyp_question.setAutoDraw(True)
    
    # if practyp_question is active this frame...
    if practyp_question.status == STARTED:
        # update params
        pass
    
    # *practyp_slider* updates
    
    # if practyp_slider is starting this frame...
    if practyp_slider.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        ...
        # update status
        practyp_slider.status = STARTED
        practyp_slider.setAutoDraw(True)
    
    # if practyp_slider is active this frame...
    if practyp_slider.status == STARTED:
        # update params
        pass
...
if practyp_resp.keys != None:  # we had a response
    ...
    thisExp.addData('practyp_resp.duration', practyp_resp.duration)
thisExp.nextEntry()
...
while continueRoutine:
    ...
    # if typfeedback_text is starting this frame...
    if typfeedback_text.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        ...
        # update status
        typfeedback_text.status = STARTED
        typfeedback_text.setAutoDraw(True)
    
    # if typfeedback_text is active this frame...
    if typfeedback_text.status == STARTED:
        # update params
        pass
    
    # if typfeedback_text is stopping this frame...
    if typfeedback_text.status == STARTED:
        if bool(typfeedback_resp.keys=='space'):
            ...
            # update status
            typfeedback_text.status = FINISHED
            typfeedback_text.setAutoDraw(False)
    
    # *typfeedback_resp* updates
    waitOnFlip = False
    
    # if typfeedback_resp is starting this frame...
    ...
    # if typfeedback_resp is stopping this frame...
    ...
    if typfeedback_resp.status == STARTED and not waitOnFlip:
        ...
        if len(_typfeedback_resp_allKeys):
            ...
            typfeedback_resp.duration = _typfeedback_resp_allKeys[-1].duration
            # a response ends the routine
            continueRoutine = False
...
if typfeedback_resp.keys != None:  # we had a response
    ...
    thisExp.addData('typfeedback_resp.duration', typfeedback_resp.duration)
thisExp.nextEntry()
...
for thisTrial in trials:
    ...
    while continueRoutine and routineTimer.getTime() < 1.0:
        ...
        # if long_crosshair is starting this frame...
        if long_crosshair.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # update status
            long_crosshair.status = STARTED
            long_crosshair.setAutoDraw(True)
        
        # if long_crosshair is active this frame...
        if long_crosshair.status == STARTED:
            # update params
            pass
        
        # if long_crosshair is stopping this frame...
        if long_crosshair.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > long_crosshair.tStartRefresh + 1.0-frameTolerance:
                ...
                # update status
                long_crosshair.status = FINISHED
    ...
    while continueRoutine:
        ...
        # if sou_resp is starting this frame...
        ...
        if sou_resp.status == STARTED and not waitOnFlip:
            ...
            if len(_sou_resp_allKeys):
                ...
                sou_resp.duration = _sou_resp_allKeys[-1].duration
                # a response ends the routine
                continueRoutine = False
        
        # *sou_pic* updates
        
        # if sou_pic is starting this frame...
        if sou_pic.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # update status
            sou_pic.status = STARTED
            sou_pic.setAutoDraw(True)
        
        # if sou_pic is active this frame...
        if sou_pic.status == STARTED:
            # update params
            pass
        
        # *sou_question* updates
        
        # if sou_question is starting this frame...
        if sou_question.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # update status
            sou_question.status = STARTED
            sou_question.setAutoDraw(True)
        
        # if sou_question is active this frame...
        if sou_question.status == STARTED:
            # update params
            pass
        
        # *sou_slider* updates
        
        # if sou_slider is starting this frame...
        if sou_slider.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # update status
            sou_slider.status = STARTED
            sou_slider.setAutoDraw(True)
        
        # if sou_slider is active this frame...
        if sou_slider.status == STARTED:
            # update params
            pass
    ...
    if sou_resp.keys != None:  # we had a response
        ...
        trials.addData('sou_resp.duration', sou_resp.duration)
    ...
    while continueRoutine:
        ...
        # if typ_resp is starting this frame...
        ...
        if typ_resp.status == STARTED and not waitOnFlip:
            ...
            if len(_typ_resp_allKeys):
                ...
                typ_resp.duration = _typ_resp_allKeys[-1].duration
                # a response ends the routine
                continueRoutine = False
        
        # *typ_pic* updates
        
        # if typ_pic is starting this frame...
        if typ_pic.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # update status
            typ_pic.status = STARTED
            typ_pic.setAutoDraw(True)
        
        # if typ_pic is active this frame...
        if typ_pic.status == STARTED:
            # update params
            pass
        
        # *typ_question* updates
        
        # if typ_question is starting this frame...
        if typ_question.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # update status
            typ_question.status = STARTED
            typ_question.setAutoDraw(True)
        
        # if typ_question is active this frame...
        if typ_question.status == STARTED:
            # update params
            pass
        
        # *typ_slider* updates
        
        # if typ_slider is starting this frame...
        if typ_slider.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            # update status
            typ_slider.status = STARTED
            typ_slider.setAutoDraw(True)
        
        # if typ_slider is active this frame...
        if typ_slider.status == STARTED:
            # update params
            pass
    ...
    if typ_resp.keys != None:  # we had a response
        ...
        trials.addData('typ_resp.duration', typ_resp.duration)
