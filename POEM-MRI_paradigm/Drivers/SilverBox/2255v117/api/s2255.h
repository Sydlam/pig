...
typedef struct s2255_device_info
{
    int boardNum;
    SN2255 serialNumber;
} DEVINFO2255;
...
typedef struct s2255_osd {
    int osdOn; //OSD on if != 0, 1=8x14 font, 2=16x16 font
    int osdChan; // OSD channel (0 to 3)
    ...
    int fraction; //Not Application: number of digits showing fraction of a second: always 0
    int xOffset, yOffset; // x offset from left side, y offset from top or bottom
    char line[160]; //caption text (use ^n for new line, ^t for time and ^d for date)
} s2255_osd;
...
#define SCALE_1CIFSF   5 // 1CIF dual field (60Hz NTSC, 50 Hz PAL)
